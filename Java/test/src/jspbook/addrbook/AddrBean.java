...
public class AddrBean { 
	...
	public boolean updateDB(AddrBook addrbook) {
		...
		String sql ="update addrbook set ab_name=?, ab_email=?, ab_birth=?, ab_tel=?, ab_comdept=?, ab_memo=?, ab_hobby=?, ab_special=?, ab_home=? where ab_id=?";		
		...
		try {
			...
			pstmt.setString(7, addrbook.getAb_hobby());
			pstmt.setString(8, addrbook.getAb_special());
			pstmt.setString(9, addrbook.getAb_home());
	...
	public boolean insertDB(AddrBook addrbook) {
		...
		String sql ="insert into addrbook(ab_name,ab_email,ab_birth,ab_tel,ab_comdept,ab_memo,ab_hobby,ab_special,ab_home) values(?,?,?,?,?,?,?,?,?)";
	...
	public AddrBook getDB(int gb_id) {
		...
		try {
			...
			addrbook.setAb_special(rs.getString("ab_special"));
			addrbook.setAb_home(rs.getString("ab_home"));
			rs.close();
		} catch (SQLException e) {
			e.printStackTrace();
		}
