...
trainData = joblib.load('temp/training.data')
trainLabels = joblib.load('temp/training.labels')
extraHipHopData = joblib.load('temp/metal.data')
extraHipHopLabels = joblib.load('temp/metal.labels')
extraClassicalData = joblib.load('temp/classical.data')
extraClassicalLabels = joblib.load('temp/classical.labels')
...
testData = []
testLabels = []
with open('temp/test.data', 'rb') as f:
  testData = pickle.load(f)
with open('temp/test.labels', 'rb') as f:
...
y = tf.placeholder(tf.float32, [None, 5]) # placeholder for corresponding labels- 4 genres
...
print(hidden_layer3)
print(hidden_layer3.shape)
# hidden_layer1 = conv_layer(x_reshaped, 1, 128, [6, 6], [6, 6], name='layer1')
# hidden_layer2 = conv_layer(hidden_layer1, 128, 64, [6, 6], [4, 4], name='layer2')
# hidden_layer3 = conv_layer(hidden_layer2, 64, 32, [4, 4], [2, 2], name='layer3')
...
wd2 = tf.Variable(tf.truncated_normal([1024, 5], stddev=0.03), name='wd2')
